...
class Landmark2LandmarkTransform(object):
    ...
    def __init__(
        self, 
        weights:Tuple[float, float, float, float, float, float, float, float, float, float]
    ):
        """
        Args:
    ...
    def get_deltas(self, scr_boxes, target_landmark):
        ...
        ``target_landmarks == self.apply_deltas(deltas, src_boxes`` is true(
        unless any delta is too large and is clamped).

        Args:
            src_boxes (Tensor): source boxes, e.g., object proposals
            target_landmarks (Tensor): target of the transormation, e.g., ground-
            truth landmarks.
        """
    ...
    def apply_deltas(self, deltas, boxes):
        """
        Apply transformation `deltas` to `landmarks`.
        
        Args:
            delta (Tensor): transformation deltas of shape (N, k*10), where k > =1.
                deltas[i] represents k potentially different class-specific 
                landmark transformtions for the single landmakr landmarks[i].
