...
def permute_all_cls_box_landmark_to_N_HWA_K_and_contat(
    box_cls, box_delta, landmark_delta, num_classes=80
):
    """
    Rearrange the tensor layout from the network output, i.e.: 
    list[Tensor]: tensor of shape(N, A*K, Hi, Wi) 
    to per-image predictions, i.e.:
    Tensor: of shape (N x sum(Hi x Wi x A), K)
    """
    # for each feature level, permute the outputs to make them be in the same
    # format as the labels. Note that the labels are computed for all feature
    # levels concatenated, so we keep the same representtion for all the 
    # objectness and the box_delta

    box_cls_flattened = [permute_to_N_HWA_K(x, num_classes) for x in box_cls]
    box_delta_flattened = [permute_to_N_HWA_K(x, 4) for x in box_delta]
    landmark_delta_flattened = [permute_to_N_HWA_K(x, 10) for x in landmark_delta]
    # contatenate on the first dimension (representing the feature levels), to 
    # take into account the way the labels were generated (with all feature maps
    # being concatenate as well)
...
class ssh_block(nn.Module):
    def __init__(self, inp, oup):
        super().__init__()
        assert oup % 4 == 0, "SSH Module required out_channels to be n*4"
        leaky = 0
        if (oup <= 64):
            leaky = 0.1
...
class RetinaFaceHead(nn.Module):
    """
    The head used in RetinaFace for object classification, box_regression and 
    landmark regression.
    """
    def __init__(self, cfg, input_shape: List[ShapeSpec]):
        super().__init__()
        ...
        for _ in range(len(input_shape)):
            ...
            landmark_pred.append(
                nn.Conv2d(in_channels, num_anchors * 10, 1, 1, 0)
            )
    ...
    def forward(self, features):
        ...
        Returns:
            logits (list[Tensor]): tensors, each has shape (N, A*K, Hi, Wi).
                The tensor predicts the classification probability at each spatial
                position for each of the A anchors and K object classes.
            box_reg (list[Tensor]): tensors each has shape (N, A*4, Hi, Wi).
                The tensor predicts 4-vector (dx, dy, dw, dh) box regression
                values for each anchor. These values are the relative offset 
                between the anchors and the ground truth box.
        """
        logits = []
        bbox_reg = []
        landmark_reg = []
...
class RetinaFace(nn.Module):
    ...
    def visualize_training(self, batched_inputs, results):
        ...
        predicted_boxes = processed_results.pred_boxes.tensor.detech().cpu().numpy()
    ...
    def losses(self,
        ...
        """
        Args:
            For `gt_classes`, `gt_anchors_deltas`, `gt_landmarks_deltas` and 
            `gt_landmarks_labels` parameters, see :meth: `RetinaNet.get_ground_truth`.
            Their shapes are (N, R), (N, R, 4), (N, R, 10) and (N, R), respectively,
            where R is the total number of anchors across levels, i.e. sum(HixWixA)
            For `pred_class_logits`, `pred_anchor_deltas` and `pred_landmark_deltas`,
            see :meth: `RetinaNetHead.forward`.
    
        Returns:
            dict[str: Tensor]:
                mapping from a named loss to a scalar tensor storing the loss. Used
                during training only. The dict keys are: "loss_cls", "loss_box_reg"
                and "loss_landmark_reg"
        """
    ...
    def get_ground_truth(self, anchors, targets):
        ...
        for targets_per_image in targets:
            ...
            if has_gt:
                ...
                matched_gt_landmarks, gt_landmarks_labels_i = matched_gt_landmarks[:, :, :2], matched_gt_landmarks[:, :,
                                                                                              2]
