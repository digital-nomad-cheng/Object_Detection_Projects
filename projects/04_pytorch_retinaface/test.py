...
parser.add_argument('-d', '--dataset', default='FFDB', help="Dataset for evaluation.")
parser.add_argument('-m', '--trained_model', default='./weights/mobilenet0.25_final_ATSS_ohem.pth',
                    type=str, help='Trained state_dict file path to open')
parser.add_argument('--network', default='resnet50', help='Backbone network mobile0.25 or resnet50')
parser.add_argument('--save_folder', default='./model_eval/fddb_evaluation/fddb_txt', type=str, help='Dir to save txt results')
parser.add_argument('--cpu', action="store_true", default=False, help='Use cpu inference')
parser.add_argument('--dataset_folder', default='./data/FFDB/', type=str, help='dataset path')
...
if __name__ == '__main__':
    ...
    for i, img_name in enumerate(image_list):
        image_path = os.path.join(dataset_folder, "images", img_name+'.jpg')
        ...
        if args.dataset == 'FFDB':
            # detection results stored in seperate file
            save_name = '_'.join(img_name.split('/')) + ".txt"
            dirname = args.save_folder + '/1'
            if not os.path.isdir(dirname):
                os.makedirs(dirname)
            with open(os.path.join(dirname, save_name), "w") as f:
                bboxs = dets
                ...
                for box in bboxs:
                    ...
                    f.write(line)
            # detection results stored in one file
            fw.write('{:s}\n'.format(img_name))
            fw.write('{:.1f}\n'.format(dets.shape[0]))
        ...
        print('im_detect: {:d}/{:d} forward_pass_time: {:.4f}s misc: {:.4f}s'.format(i + 1, num_images,
                                                                                     timer['forward_pass'].average_time,
