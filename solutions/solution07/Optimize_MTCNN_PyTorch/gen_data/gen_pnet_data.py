"""
Generate positive, negative, positive size PNET_SIZE*PNET_SIZE images for PNet
"""
import os, sys
sys.path.append('.')
import cv2
...
def generate_pnet_data(cfg, mode='train'):
    net_size = cfg.MODEL.PNET_SIZE
    prefix = ''
    ...
    for annotation in annotations:
        ...
        # annotation form is x1, y1, w, h, convert to x1, y1, x2, y2
        boxes[:, 2] += boxes[:, 0] - 1
        boxes[:, 3] += boxes[:, 1] - 1
        ...
        for box in boxes:
            ...
            if max(w, h) < cfg.DATA.smallest_face_size or x1 < 0 or y1 < 0 or w < 0 or h < 0:
                continue
...
if __name__ == "__main__":
    # generate_pnet_data(config, 'train')
    # generate_pnet_data(config, 'val')